...
3. AI inference và Computer Vision chạy bên ngoài backend này. Frontend gọi trực tiếp các AI service đó và chỉ gửi sự kiện, điểm số, thời lượng và kết quả đã suy ra về backend.
...
15. Trẻ chỉ được nhận sao một lần cho mỗi content. Các lần hoàn thành sau vẫn có thể ghi nhận tiến độ/lịch sử nhưng phải cộng 0 sao.
...
Frontend gọi trực tiếp AI/computer-vision service. Backend này không gọi model, không biết topology/model version, và không giữ credential inference service.
...
- **Suy luận** `metadata` cho rage-click count, trạng thái nhận diện khuôn mặt hoặc ngữ cảnh quan sát của frontend.
...
- Priority: P0.
- Current status: Một phần. Đã có signup email/password kèm session ngay. Thiếu Google registration/linking.
...
- Main flow:
  ...
  3. Frontend gọi AI service bên ngoài và nhận match score/confidence.
  4. Client gửi kết quả AI game đã suy ra.
  5. Backend validate score range, threshold policy, unlock state và first-reward rule.
  6. Backend ghi session và thưởng sao nếu score đạt ngưỡng và trẻ chưa từng nhận sao cho game đó.
- Alternative/error flows:
...
- Priority: P1.
- Current status: Một phần. Có field DB. Thiếu route/use case; backend chỉ nên lưu kết quả AI đã suy ra.
...
- Main flow:
  1. Luồng AI bên ngoài xác định tín hiệu tiêu cực vượt ngưỡng cấu hình.
...
- Actors: Luồng AI bên ngoài, child-facing client.
- Preconditions: AI xác định tín hiệu tiêu cực đạt ngưỡng.
- Main flow:
...
| `POST`   | `/auth/google`                 | Authenticate/register bằng Google SSO, link local account cùng email đã verify. | Public.                            | Body: `id_token` hoặc OAuth `authorization_code`, optional `redirect_uri`. | `200` hoặc `201` với `user` và `session`.               | `400` request không hợp lệ. `401` Google credential không hợp lệ. `403` account banned. `409` provider conflict.   | Verify issuer, audience, expiry, email verification, provider ID uniqueness; link cùng email đã verify vào local account.                 | UC-AUTH-01, UC-AUTH-02 | Thiếu                             |
...
| `PATCH`  | `/children/:childId`             | Cập nhật child profile.                 | Parent sở hữu child.  | Path: `childId`. Multipart body với optional `nickname`, file `avatar`. | `200` với `child` đã cập nhật; `avatar_url` là presigned URL.  | `400` body/UUID/avatar không hợp lệ. `401` invalid session. `403` không sở hữu child. `404` child not found. `413` avatar quá lớn. `502` upload S3 lỗi.                | Giới hạn field mutable như create; phải có ít nhất một field. `birth_year` chỉ được set khi tạo từ frontend.                                             | UC-CHILD-03           | Thiếu         |
...
| Method | Path                                     | Mục đích                                            | Auth/Authz                                   | Request                                                                                                                 | Response                                                      | Error responses                                                                                                          | Validation                                                                                                                                                          | Use case liên quan                  | Trạng thái                 |
| ------ | ---------------------------------------- | --------------------------------------------------- | -------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------ | ------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ----------------------------------- | -------------------------- |
| `POST` | `/children/:childId/emotion-logs`        | Ghi nhận emotion/behavior event đã suy ra.          | Parent sở hữu child.                         | Body: `emotion_value`, `trigger_source`, optional `duration_seconds`, optional suy luận `confidence_score`, `metadata`. | `201` với `log`.                                              | `400` UUID/emotion/source/duration không hợp lệ. `401` invalid session. `403` không sở hữu child. `404` child not found. | Emotion/source enum; duration là số nguyên dương; confidence 0-1 hoặc 0-100, chọn một convention và enforce. Backend không gọi AI service.                          | UC-TRACK-01, UC-REG-01, UC-NOTIF-01 | Đã triển khai basic create |
...
| `DELETE` | `/admin/pets/:petId`         | Soft-delete hoặc hide pet.                 | Admin authenticated. | Path: `petId`; body confirmation hoặc `mode`.                                                                       | `200` message.                                          | `400` UUID/confirmation không hợp lệ. `401` invalid session. `403` không phải admin. `404` not found.                | Ưu tiên `status = HIDDEN` khi cần hide có thể đảo ngược hoặc set `deleted_at` khi delete; giữ lịch sử child ownership.      | UC-ADMIN-02              | Thiếu           |
...
2. Route request/verify/confirm reset mật khẩu.
3. Route cập nhật account và đổi mật khẩu.
4. Đăng ký device token và gửi push notification.
5. Route detail, update và delete child profile.
6. Route read/update preferences và typed preference schema.
7. Route browse/detail content cho UI học tập của trẻ.
8. Route content session completion cho lecture, quiz, game và abandoned session.
9. Giá trị thưởng sao và xử lý idempotent reward.
10. Route dùng sao để unlock content và mua pet.
11. Star ledger bất biến để debug biến động balance.
12. Pet store và route quản lý pet của trẻ.
13. Data model quiz answer options.
14. AI game scoring contract và success threshold.
15. Emotion log listing/filtering.
16. Regulation event model và route.
17. Alert model, alert cooldown và tracking notification status.
...
24. Audit logging cho admin changes, soft deletes, bans và report exports.
25. Pagination/cursor support cho list endpoints.
26. Rate limiting cho auth, password reset, emotion logging và AI event ingestion.
27. RBAC middleware cho parent/admin/system authorization.
28. Webcam consent và enforcement chính sách retention dữ liệu.
...
1. Giá trị sao cụ thể cho hoàn thành lecture, trả lời quiz đúng và thắng AI game là bao nhiêu?
2. Mô hình quiz answer option nên là fixed emotion enum, option do admin định nghĩa theo từng quiz, hay option text/media có localization theo quiz?
3. Emotion taxonomy chuẩn cho quiz, emotion log, AI game target và report là gì?
4. Webcam frame thô có bao giờ được upload/lưu ở bất kỳ đâu không, hay product policy phải cấm hoàn toàn upload/lưu raw frame?
5. Ngưỡng score/confidence nào xác định AI game thành công?
6. Ngưỡng stress/meltdown ngoài rule SRS "cảm xúc tiêu cực hơn 1 phút" là gì?
...
6. Không tin `stars_earned` do client gửi. Backend phải tự tính reward từ content policy và kết quả đã validate, và chỉ thưởng sao tối đa một lần mỗi `(child_id, content_id)`.
...
10. Xem AI score là outcome đã suy ra do frontend gửi. Backend này không gọi AI service trực tiếp, nên endpoint reward cần validate score range chặt, idempotency và first-reward enforcement.
