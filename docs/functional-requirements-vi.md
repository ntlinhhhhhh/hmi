...
| `POST`   | `/auth/password-reset/request` | Yêu cầu OTP/link reset mật khẩu qua email hoặc SMS.                             | Public.                            | Body: `identifier` email hoặc phone.                                       | `200` generic message.                                  | `400` identifier không hợp lệ. `500` thiếu SMS config. `502` SMS provider lỗi. `429` quá nhiều request.            | Không tiết lộ account có tồn tại hay không; rate-limit theo identifier/IP; chỉ cho local/phone account.                                   | UC-AUTH-04             | Một phần: thiếu rate limit.            |
...
3. Đăng ký device token và gửi push notification.
4. Route read/update preferences và typed preference schema.
5. Route browse/detail content cho UI học tập của trẻ.
6. Route content session completion cho lecture, quiz, game và abandoned session.
7. Use case idempotency cho content session quanh reward sao cố định.
8. Route dùng sao để unlock content.
9. Star ledger bất biến để debug biến động balance.
10. AI game scoring contract và detection confidence threshold.
11. Emotion catalog cuối cùng.
12. Emotion log listing/filtering.
13. Regulation event model và route.
14. Alert model, alert cooldown và tracking notification status.
...
21. Audit logging cho admin changes, soft deletes, bans và report exports.
22. Pagination/cursor support cho list endpoints.
23. Rate limiting cho auth, password reset, emotion logging và AI event ingestion.
24. RBAC middleware cho parent/admin/system authorization.
25. Webcam consent và raw-frame ban enforcement.
